...
# perform summary analysis on the ss dataframe
print(ss.describe())
...
X = ss.drop(columns=['sm_li'])  # Feature set
...
# Create a DataFrame with informative labels
pd.DataFrame(confusion_matrix(y_test, y_pred),
            columns=["Predicted negative", "Predicted positive"],
            index=["Actual negative","Actual positive"]).style.background_gradient(cmap="PiYG")
...
newdata["prediction_pro"]=logistic_model.predict(newdata)
...
person1 = [8,7,0,1,0,42]
person2 = [8,7,0,1,0,82]
...
predicted_class1 = logistic_model.predict([person1])
predicted_class2 = logistic_model.predict([person2])

# Generate probability of positive class (=1)
probs1 = logistic_model.predict_proba([person1])
probs2 = logistic_model.predict_proba([person2])
...
print(f"Predicted class1: {predicted_class1[0]}") 
print(f"Probability of LinkedIn usage (42 year old female): {probs1[0][1]}")


# In[93]:


print(f"Predicted class: {predicted_class2[0]}") 
print(f"Probability of LinkedIn usage (82 year old female): {probs2[0][1]}")
